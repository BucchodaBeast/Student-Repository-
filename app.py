...
def index():
    ...
    topics = db.execute(
        "SELECT DISTINCT topic_area FROM projects ORDER BY topic_area"
    ).fetchall()
    ...
    return render_template(
        ...
        query=query,
        scores=scores,
        topics=[t["topic_area"] for t in topics],
...
def submit():
    ...
    if request.method == "POST":
        ...
        if not all([title, abstract, student_name, supervisor, keywords, topic_area, year]):
        ...
        cur = db.execute(
            """INSERT INTO projects
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (title, abstract, student_name, supervisor, keywords, topic_area, int(year),
             file_path, summary, builds_on_id, datetime.now().isoformat()),
        )
    ...
    return render_template("submit.html", all_projects=all_projects, form={})
