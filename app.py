...
PUBLIC_ROLES = ["student", "researcher"]  # roles selectable at self-registration
...
def register():
    ...
    if request.method == "POST":
        ...
        if not all([name, email, password]) or role not in PUBLIC_ROLES:
            flash("Please fill in every field.", "error")
            return render_template("register.html", form=request.form, roles=PUBLIC_ROLES)

        if len(password) < 6:
            flash("Password must be at least 6 characters.", "error")
            return render_template("register.html", form=request.form, roles=PUBLIC_ROLES)

        # Admin accounts can't be self-selected from the role picker — only an
        # invite code known to real administrators grants the admin role.
        if invite_code:
            if invite_code != ADMIN_INVITE_CODE:
                flash("That admin invite code isn't valid.", "error")
                return render_template("register.html", form=request.form, roles=PUBLIC_ROLES)
            role = "admin"
    ...
    return render_template("register.html", form={}, roles=PUBLIC_ROLES)
...
def review_queue():
    ...
    return render_template("review_queue.html", pending=pending, recently_reviewed=recently_reviewed)
...
def review_project(project_id):
    ...
    db.execute(
        "UPDATE projects SET approval_status = ?, review_note = ?, reviewed_by = ?, reviewed_at = ? WHERE id = ?",
        (new_status, note or None, current_user.id, datetime.now().isoformat(), project_id),
    )
    db.commit()
    flash(f"'{project['title']}' was {new_status}.", "success")
    return redirect(url_for("review_queue"))
