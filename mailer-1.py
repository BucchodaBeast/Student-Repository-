...
they're not set yet:
    ...
    SMTP_USER         the Gmail address to send from (e.g. launchpad@gmail.com)
    ...
    SMTP_HOST         defaults to smtp.gmail.com (override for a different
                      provider — SendGrid, Mailgun, etc. all speak SMTP)
...
If SMTP_USER/SMTP_PASS aren't set, every send_email() call is a harmless
no-op that just logs to stdout — the same "off unless configured" pattern
already used for GROQ_API_KEY and Google OAuth.
"""
